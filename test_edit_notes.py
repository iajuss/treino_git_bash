...
class EditNotesTest(unittest.TestCase):
    ...
    def test_note_card_exposes_edit_and_delete_actions(self):
        page = self.client.get("/").get_data(as_text=True)

        self.assertIn('name="edit_button"', page)
        self.assertIn('href="/update/1"', page)
        self.assertIn('name="delete_button"', page)

        response = self.client.get("/delete/1")
        self.assertEqual(response.status_code, 302)

        connection = sqlite3.connect("banco.db")
        note = connection.execute("SELECT id FROM note WHERE id = 1").fetchone()
        connection.close()
